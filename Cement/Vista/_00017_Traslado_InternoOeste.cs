...
{
    public partial class _00017_Traslado_InternoOeste : Form
    {
       
        public _00017_Traslado_InternoOeste()
        {
            InitializeComponent();
            txtBusqueda.Clear();
            cargarCodCemInt();
        ...
        private void _00017_Interno_Load(object sender, EventArgs e)
        {
            dgvInterno.DataSource = objCausante.MostrarCausantesSinTrasladosExt();
            dgvInterno.Columns["TrasladoExterno"].Visible = false;
        ...
        public void cargarCodCemInt()
        {
            try
            {
                ...
                this.cmbCodCemInt.DataSource = objNomencOeste.MostrarTodos();//usando la clase nomenc
        ...
        private void btnTrasladar_Click(object sender, EventArgs e)
        {
            ...
            {
                _00018_Traslado_Interno_Oeste objForm = new _00018_Traslado_Interno_Oeste(this.objCausante);//enviar como parametro un nuevo causante al constructor de la clase.
                objForm.Text = "TRASLADO INTERNO OESTE";
        ...
        private void cmbCodCemInt_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (cmbCodCemInt.SelectedText != "OESTE")
            {
        ...
        private void btnEliminar_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(this.objCausante.ToString()))
            {
                _00018_Traslado_Interno_Oeste objForm = new _00018_Traslado_Interno_Oeste(this.objCausante);//enviar como parametro un nuevo causante 

                objForm.cmbCodCem.Enabled = false;
                objForm.btnNuevoCement.Enabled = false;
                objForm.cmbTipoConst.Enabled = false;
                objForm.cmbTramiteInt.Enabled = false;
                objForm.txtSeccion.Enabled = false;
                objForm.txtCuadro.Enabled = false;
                objForm.txtZona.Enabled = false;
                objForm.txtTamNicho.Enabled = false;
        ...
        private void txtCuadroInt_TextChanged(object sender, EventArgs e)
        {
            try
            {
                BindingSource objBindingSource = new BindingSource();
                objBindingSource.DataSource = this.dgvInterno.DataSource;
                objBindingSource.Filter = "Codcem = '" + this.cmbCodCemInt.Text + "' AND Cuadro LIKE '%" + this.txtCuadroInt.Text + "%' AND Zona LIKE '%" + this.txtZonaInt.Text + "%' AND Lote LIKE '%" + this.txtLoteInt.Text + "%'";
                this.dgvInterno.DataSource = objBindingSource.DataSource;
            }
            catch (Exception ex)
            {
