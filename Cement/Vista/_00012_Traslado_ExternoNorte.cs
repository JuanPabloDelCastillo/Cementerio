...
namespace Cement.Vista
{
    public partial class _00012_Traslado_ExternoNorte : Form
    {
        ...
        public _00012_Traslado_ExternoNorte()
        {
        }
        ...
        private void txtResponsable_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyValue == (char)Keys.F2)
            {
                ...
                {
                    this.txtidEmpleado.Text = EmpTipo.dgvUSUARIOS.CurrentRow.Cells["id_Empleado"].Value.ToString();
                    this.txtResponsable.Text = EmpTipo.dgvUSUARIOS.CurrentRow.Cells["ColumnNombre_E"].Value.ToString();
                    this.txtEmpNumAfil.Text = EmpTipo.dgvUSUARIOS.CurrentRow.Cells["ColumnAfiliado_Num"].Value.ToString();
                }
            }
        }
        ...
        private void txtMeses_Validating(object sender, CancelEventArgs e)
        {
            try
            {
                int limiteMeses = 12;
                int limiteActual = Convert.ToInt32(txtMeses.Text);
                if (string.IsNullOrEmpty(txtMeses.Text))
                {
                    txtEdad.Enabled = true;

                    //  limiteactualEdad = limiteEdad;
                }
                if (limiteActual > limiteMeses)
                {
                    MessageBox.Show("La cantidad de Meses no puede superar los 12");
                    txtMeses.Focus();
                }
            }
            catch
        ...
        private void _00012_Traslado_ExternoNorte_Load_1(object sender, EventArgs e)
        {
            btnGuardar.Enabled = false;
        }

        private void cmbTraslado_Leave(object sender, EventArgs e)
        {
        ...
        {
            _000022_TrasladoExNorte objNorte = new _000022_TrasladoExNorte();
            objNorte.btnTrasladar.Enabled = false;
            this.Close();
